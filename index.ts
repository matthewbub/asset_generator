...
function loadSystemPromptConfig(): SystemPromptConfig {
  try {
    ...
    console.warn(
      "Warning: Could not load system prompt config, using defaults"
    );
  }
...
async function generateAssetWithResponses(
  prompt: string,
  previousImageCallId?: string
): Promise<ImageGenerationResult> {
  const config = loadSystemPromptConfig();
  const finalPrompt = config.enabled
    ? combinePrompts(prompt, config.systemPrompt)
    : prompt;
...
async function generateAssetLegacy(
  prompt: string
): Promise<ImageGenerationResult> {
  const config = loadSystemPromptConfig();
  const finalPrompt = config.enabled
...
function listGeneratedImages(): string[] {
  ...
  return files.filter(
    (file) => file.startsWith("generated_") && file.endsWith(".png")
  );
}
  ...
  switch (action.action) {
    case "Set system prompt":
      ...
      saveSystemPromptConfig({
        systemPrompt: promptAnswer.prompt,
        enabled: true,
    ...
    case "Enable max size mode":
      ...
      console.log(
        "✅ Max size mode enabled! (Legacy API will use 1792x1024 or 1024x1792)"
      );
      break;
...
async function startInteractiveSession() {
  ...
  while (true) {
    ...
    if (modeAnswer.mode === "Refine existing image") {
      ...
      selectedImageCallId =
        extractImageCallIdFromFilename(imageAnswer.image) || undefined;
    ...
    try {
      if (modeAnswer.mode === "Use legacy API") {
        ...
        lastImageResult = await generateAssetWithResponses(
          promptAnswer.prompt,
          selectedImageCallId
        );
      } else {
